...
class People(hpb.BasePeople):
    ...
    **Examples**::
    ...
    def infect(self, inds, genotypes=None, source=None, offset=None, dur_inf=None, layer=None):
        '''
        ...
        if dur_inf is None:
            ...
            if len(dur_inf) != len(inds):
                errormsg = f'If supplying infection durations, they must be the same length as inds: {len(dur_inf)} vs. {len(inds)}.'
        ...
        for g in range(ng):
            ...
            cin1_inds, dur_cin1, dur_cin1_inds = self.make_cin1(g,      inds, cin1_probs,  dur_inf, base_t, dt, durpars['cin1'], durpars['hpv_post_cin1'])

            cin2_probs = progprobs[g]['rel_cin2_prob'] * progpars['cin2_probs'][dur_cin1_inds]
            cin2_inds, dur_cin2, dur_cin2_inds = self.make_cin2(g, cin1_inds, cin2_probs, dur_cin1, base_t, dt, durpars['cin2'])

            cin3_probs = progprobs[g]['rel_cin3_prob'] * progpars['cin3_probs'][dur_cin2_inds]
            cin3_inds, dur_cin3, dur_cin3_inds = self.make_cin3(g, cin2_inds, cin3_probs, dur_cin2, base_t, dt, durpars['cin3'])

            cancer_probs = progprobs[g]['rel_cancer_prob'] * progpars['cancer_probs'][dur_cin3_inds]
            cancer_inds, dur_cancer, dur_cancer_inds = self.make_cancer(g, cin3_inds, cancer_probs, dur_cin3, base_t, dt, durpars['cin1'])

            # Update immunity
            hpi.update_peak_immunity(self, inds, imm_pars=self.pars, imm_source=g, offset=offset)

        # Store the overal duration of infection and the date of HPV clearance
        self.dur_inf[genotypes, inds] = dur_inf
        self.date_hpv_clearance[genotypes, inds] = base_t + np.ceil(dur_inf / dt)  # Date they clear HPV infection (interpreted as the timestep on which they recover)

        return new_infections


    def make_cin1(self, g, inds, cin1_probs, dur_inf, base_t, dt, dur_cin1, dur_hpv_post_cin1):
        ''' Make CIN1 '''
        ...
        durs_hpv2cin1 = hpu.sample(**dur_hpv_post_cin1, size=len(cin1_inds))  # Store how long this person took to develop CIN1
        ...
        excl_inds = hpu.true(self.date_cin1[g, cin1_inds] < base_t) # Don't count CIN1s that were acquired before now
        ...
        dur_cin1 = hpu.sample(**dur_cin1, size=len(cin1_inds))  # Duration of infection in YEARS
        dur_cin1_inds = np.digitize(dur_cin1, progpars['duration_cutoffs']) - 1  # Convert durations to indices

        return cin1_inds, dur_cin1, dur_cin1_inds


    def make_cin2(self, g, cin1_inds, cin2_probs, dur_cin1, base_t, dt, dur_cin2):
        '''
        Make CIN2
        '''
        ...
        excl_inds = hpu.true(self.date_cin2[g, cin2_inds] < base_t) # Don't count CIN2s that were acquired before now
        ...
        dur_cin2 = hpu.sample(**dur_cin2, size=n_cin2_inds)  # Duration of infection in YEARS
        dur_cin2_inds = np.digitize(dur_cin2, progpars['duration_cutoffs']) - 1  # Convert durations to indices

        return cin2_inds, dur_cin2, dur_cin2_inds


    def make_cin3(self, g, cin2_inds, cin3_probs, dur_cin2, base_t, dt, dur_cin3):
        ''' Make CIN3 '''
        ...
        # CIN2 with progression to CIN3
        n_cin3_inds = len(cin3_inds)
        self.dur_cin22cin3[g, cin3_inds] = dur_cin2[is_cin3]
        excl_inds = hpu.true(self.date_cin3[g, cin3_inds] < base_t) # Don't count CIN2s that were acquired before now
        ...
        dur_cin3 = hpu.sample(**dur_cin3, size=n_cin3_inds)  # Duration of infection in YEARS
        dur_cin3_inds = np.digitize(dur_cin3, progpars['duration_cutoffs']) - 1  # Convert durations to indices

        return cin3_inds, dur_cin3, dur_cin3_inds


    def make_cancer(self, g, cin3_inds, cancer_probs, dur_cin3, base_t, dt, dur_cancer):
        ''' Make cancer '''
        ...
        excl_inds = hpu.true(self.date_cancerous[g, cancer_inds] < base_t) # Don't count cancers that were acquired before now
    ...
    def make_cin(self, inds, genotypes=None):
        ''' Give people CIN. Not generally used outside of model initialization '''
