    ...
    **Example**::
    ...
    def run(self, debug=False, keep_people=False, verbose=None, **kwargs):
        ...
        for scenkey,scen in self.scenarios.items():
            ...
            # CK: TODO: need to refactor to not load this again!
            location = allpars['location']
            data_pars = scen_sim.load_hiv_data(location=location, hiv_datafile=scen_sim.hiv_datafile, art_datafile=scen_sim.art_datafile)  # Load any data that's used to create additional parameters (thus far, HIV and ART)
