...
noneshares, cin1shares, cin2shares, cin3shares = [], [], [], []
longx = np.linspace(0.01, 20, 1000)
for g in range(ng):
    sigma, scale = lognorm_params(durpars[g]['dys']['par1'], durpars[g]['dys']['par2'])
    rv = lognorm(sigma, 0, scale)
    dd = mean_peak_fn(longx, genotype_pars[genotype_map[g]]['prog_rate'])[1:]
...
def make_fig1():
    ...
    # ax[0,2].bar(np.arange(ng)+1, shares)
    # ax[0,2].set_xticks(np.arange(ng)+1)
    # ax[0,2].set_xticklabels(gtypes)
    # ax[0,2].set_ylabel("")
    ...
    ax[1,0].set_ylabel("")
    ax[1,0].grid(axis='x')
    ax[1,0].set_title("Mean peak clinical severity by duration\n(dots show individual variation)")
    ...
    for gn,grade in enumerate(['CIN1', 'CIN2', 'CIN3']):
        ...
        ax[1,1].bar(years, ydata, color=cmap[gn], bottom=bottom, label=grade);
        bottom = bottom + ydata
    ax[1,1].legend()
    ax[1,1].set_title("Share of women with HPV16\ndysplasia by clinical grade")
    ax[1,1].set_xlabel("Duration post-dysplasia")
    ...
    for gn,grade in enumerate(['None', 'CIN1', 'CIN2', 'CIN3']):
