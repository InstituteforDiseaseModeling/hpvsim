...
if __name__ == '__main__':
    ...
    # num_people = int(2e4) #int(2e3) called num_arr and num_vals in post
    # num_contacts = int(1e4) #int(1e4) called N in post
    # num_infections = int(1e3) #int(1e4) called M in post
    # num_arr = 100e3 #int(2e4)
    # num_vals = 20e3
    # num_types = 6
    # arr = np.random.randint(0, num_people, num_contacts)
    # vals_col1 = np.random.randint(0, num_people, num_infections)
    # vals_col2 = np.random.randint(0, num_types, num_infections)
    # vals = np.array([vals_col1, vals_col2])
    #
    # # @nb.njit  # tested with numba 0.55.1
    # def isin_nb(arr, vals, num_people):
    #     lookup = np.empty(num_people, np.float32)
    #     lookup.fill(np.nan)
    #     lookup[vals[0, ::-1]] = vals[1, ::-1]
    #     res_val = lookup[arr]
    #     return ~np.isnan(res_val), res_val
    #
    # res, res_val = isin_nb(arr, vals, num_people)

    # N = int(10e3) #int(1e4) called N in post
    # M = int(20e3) #int(1e4) called M in post
    # num_arr = int(100e3) #int(2e3) called num_arr and num_vals in post
    # num_vals = int(20e3)
    # num_types = 6
    ...
    #
    # # @nb.njit  # tested with numba 0.55.1
    # def isin_nb(arr, vals, num_vals):
    #     lookup = np.empty(num_arr, np.float32)
    #     lookup.fill(np.nan)
    #     lookup[vals[0, ::-1]] = vals[1, ::-1]
    #     res_val = lookup[arr]
    #     return ~np.isnan(res_val), res_val
    #
    # res, res_val = isin_nb(arr, vals, num_vals)
