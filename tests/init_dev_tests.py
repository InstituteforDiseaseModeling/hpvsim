...
def test_genotypes():
    ...
    for i, genotype in sim['genotype_map'].items():
        ax[0].plot(timevec, sim.results['genotype']['new_infections_by_genotype'].values[i,:], label=genotype)
        ax[1].plot(timevec, sim.results['genotype']['new_precancers_by_genotype'].values[i,:])

    ax[0].legend()
    ax[1].set_title('New CIN by genotype')
    ax[0].set_title('New infections by genotype')
    fig.show()
    return sim
